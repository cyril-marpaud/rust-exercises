...
impl NeuralNetwork {
	...
	fn backward(&mut self, target: f64, activations: &[Vec<f64>], learning_rate: f64) {
		let output = &activations[activations.len() - 1][0];
		let mut error = output - target; // Dérivée MSE
		...
		for (layer_idx, layer) in self.layers.iter_mut().enumerate().rev() {
				...
				layer.weights.iter_mut().zip(&mut layer.biases).enumerate()
			{
				...
				let delta = error * layer.activation.derivative(output_activation[i]);
				...
				*bias -= learning_rate * delta;
				...
				for (j, weight) in weight_row.iter_mut().enumerate() {
					...
					*weight -= learning_rate * delta * input_activation[j];
				}
			}
	...
	pub fn train(
		...
		for epoch in 0..epochs {
			...
			for (input, target) in inputs.iter().zip(targets) {
				...
				// Calcul de l'erreur
				let error = 0.5 * (output[0] - target).powi(2);
				total_error += error;
